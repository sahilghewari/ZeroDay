...
class App {
  ...
  setSize(width: number, height: number, updateStyles: boolean) {
    // Ensure renderer and composer sizes stay in sync.
    try {
      this.renderer.setSize(width, height, updateStyles);
      // Update canvas style when requested so CSS layout matches drawing buffer
      if (updateStyles && this.renderer.domElement) {
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = {} }) => {
  ...
  useEffect(() => {
    ...
    // Load assets and initialize. After init, ensure renderer size matches the container
    // (some browsers/layouts may not have final sizes immediately on init) so run a delayed resize.
    myApp.loadAssets().then(() => {
      myApp.init();
      // ensure the canvas fills the container
      try {
        const canvas = myApp.renderer.domElement as HTMLCanvasElement;
      ...
      // schedule a resize after layout stabilizes
      setTimeout(() => {
        try {
          myApp.setSize(container.clientWidth || container.offsetWidth, container.clientHeight || container.offsetHeight, true);
        } catch (e) {
          // ignore
        }
      }, 50);

      // Observe container size changes and update renderer accordingly
        ...
        ro = new ResizeObserver(entries => {
          for (const entry of entries) {
            ...
            if (cr.width && cr.height) {
              try {
                myApp.setSize(Math.floor(cr.width), Math.floor(cr.height), true);
              } catch (e) {}
            }
          }
        });
      ...
      // store observer on the app for cleanup
      (myApp as any).__resizeObserver = ro;
    });
    ...
    return () => {
      ...
      // disconnect any ResizeObserver we attached
      try {
        const ro = (appRef.current as any)?.__resizeObserver as ResizeObserver | null;
        if (ro) ro.disconnect();
      } catch (e) {}
    };
  }, [mergedOptions]);
